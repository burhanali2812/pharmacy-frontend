...
function App() {
  ...
  useEffect(() => {
    ...
    const fetchMedicines = async () => {
      try {
        const response = await fetch("http://localhost:5000/auth/get-medicine", {
          method: "GET",
          headers: {
  ...
  setPreExpiredMedicines(preExpire);
    ...
    const lStock = medicines.filter(med => med.quantity > 0 && med.quantity < 10);
    setLowStock(lStock);
  },[medicines])
        ...
        <Route
          ...
          element={
            <Sidebar cart={cart} role={role} user={user} >
              <Routes>
                <Route path="/search" element={<Search cart={cart} setCart={setCart} medicines={medicines} setMedicines={setMedicines} role = {role}/>} />
                <Route path="/addMedicine" element={<AddMedicine medicines={medicines} setMedicines={setMedicines} />} />
                <Route path="/checkout" element={<CheckOut cart={cart} setCart={setCart} medicines={medicines} />} />
                <Route path="/prescription" element={<Prescription medicines={medicines} />} />
                <Route path="/dashboard" element={ <Dashboard medicines={medicines} expireMedicines ={expireMedicines} outOfStock ={outOfStock}
                lowStock = {lowStock} salesManUsers ={salesManUsers} preExpireMedicines={preExpireMedicines}/>  }/>
                <Route path="/buy" element={<BuyMedicine cart={cart} setCart={setCart} medicines={medicines} setMedicines={setMedicines} />} />
                <Route path="/bill" element={<Bill />} />
                <Route path="/userInvoice" element={<UserInvoices role = {role}/>} />
                <Route path="/bill-Invoice/:id" element={<Bill_invoice />} />
                <Route path="/past-suppliers" element={<PastSupplier />} />
                <Route path="/details-suppliers/:supplierId" element={<SupplierDetails />} />
                <Route path="/suppliers" element={<SupplierShow  role = {role}/>} />
                <Route path="/expired-medicines" element={<ExpiredMedicines expireMedicines ={expireMedicines}/>} />
                <Route path="/low-stock" element={<LowStockMedicines lowStock = {lowStock}/>}/>
                <Route path="/outOfStock" element={<OutOfStockMedicines outOfStock ={outOfStock} />} />

...
function Search({ cart, setCart, medicines, setMedicines, role }) {
    ...
    const openEditModal = (medicine) => {
        ...
        const editModal = new window.bootstrap.Modal(document.getElementById("editMedicineModal"));
        editModal.show();
    };
    const handleUpdateSupplier = async () => {
        ...
        const medicine = supplierMedicine.find((med) => med.sID === selectedMedicine.sID);
        ...
        await fetchMedicines();
        const selectedSupplierMedicines = supplierMedicine.filter((sup) => sup.supplier === medicine.supplier);
        const selectedSupplierID = suppliers.find((sup) => sup.name === medicine.supplier);
        ...
        const totalMedicineQuantity = selectedSupplierMedicines.reduce((sum, med) => sum + med.quantity, 0);
        const totalMedicineCost = selectedSupplierMedicines.reduce((sum, med) => sum + (med.quantity * med.actualPrice), 0);
        ...
        const newQuantity = existingSupplierQuantity + (updatedMedicineQuantity - existingMedicineQuantity);
        const newTotalMEdicinequantity = totalMedicineQuantity + (updatedMedicineQuantity - existingMedicineQuantity);
        const newTotalMEdicineCost = Number(totalMedicineCost) + (updatedMedicineQuantity - existingMedicineQuantity) * existingMedicineCost;
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5000/auth/update-quantity/${selectedSupplierID._id}`, {
                ...
                body: JSON.stringify({ additionalQuantity: newTotalMEdicinequantity, totalPrice: newTotalMEdicineCost })
            });
        ...
        try {
            const response = await fetch(`http://localhost:5000/auth/update-supplier-medicine/${medicine._id}`, {
                method: "PUT",
                headers: {
                    Authorization: `Bearer ${token}`,
                    "Content-Type": "application/json",
            ...
            showToast("success", "Supplier medicine info updated successfully!", 3000);
            await fetchMedicines();
    ...
    const handleDeleteAllMedicines = async () => {
        ...
        try {
            const response = await fetch('http://localhost:5000/auth/delete-all-medicines', {
                method: "DELETE",
                headers: {
                    Authorization: `Bearer ${token}`,
                    "Content-Type": "application/json",
            ...
            if (response.ok) {
                ...
                showToast("error", `Failed to delete medicines: ${data.error || "Unknown error"}`, 3000);
            }
        } catch (error) {
    ...
    const handleUpdate = async (e) => {
        ...
        try {
            const response = await fetch(`http://localhost:5000/auth/update-medicine/${selectedMedicine._id}`, {
                method: "PUT",
                headers: {
                    Authorization: `Bearer ${token}`,
                    "Content-Type": "application/json",
            ...
            setMedicines((prev) =>
                prev.map((med) => (med._id === selectedMedicine._id ? selectedMedicine : med))
            );
    ...
    const handleDelete = async (medicineDel) => {
        ...
        try {
            const response = await fetch(`http://localhost:5000/auth/delete-medicine/${medicineDel._id}`, {
                method: "DELETE",
                headers: {
                    Authorization: `Bearer ${token}`,
                    "Content-Type": "application/json",
    ...
    const exportToPDF = () => {
        ...
        const tableRows = medicines.map(med => [med.sID, med.name, med.quantity, med.price, med.supplier, med.expire]);
    ...
    return (
        <div>
            ...
            <div className="container">
                ...
                {role === 'admin' ? (<div className="d-flex justify-content-end" style={{ marginTop: 40 }}>
                    <button className="btn btn-outline-dark position-relative" type="button" onClick={handleDeleteAllMedicines}>
                            ...
                            filteredMedicines.map((med, index) => {
                                ...
                                return (
                                    <tr key={med._id} className={stockClass}>
                                        ...
                                        <td>{med.expire}</td>
                                        <td>
                                            <button className="btn btn-outline-success btn-sm mx-1" onClick={() => openEditModal(med)}>
                                                <i className="fas fa-edit"></i>
                                            </button>
                                            <button className="btn btn-outline-danger btn-sm mx-1" onClick={() => handleDelete(med)}>
            ...
            <div className="modal fade" id="editMedicineModal" tabIndex="-1" aria-labelledby="editMedicineModalLabel" aria-hidden="true">
                <div className="modal-dialog">
                    <div className="modal-content">
                        <div className="modal-header">
                            <h5 className="modal-title" id="editMedicineModalLabel">Edit Medicine</h5>
                                ...
                                <form onSubmit={handleUpdate}>
                                    ...
                                    <div className="mb-3">
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => setSelectedMedicine({ ...selectedMedicine, expire: e.target.value })}
                                            required
                                        />
                                    </div>

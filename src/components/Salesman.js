...
function Salesman() {
    ...
    const makeAdmin = async (user) => {
        ...
        try {
            const response = await fetch(`http://localhost:5000/auth/update-user-role/${user._id}`, {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${localStorage.getItem("token")}`
            ...
            if (response.ok) {
                ...
                if (user.email === localStorage.getItem("email") && newRole !== "admin") {
                    showToast("success", "Your admin access has been revoked. Logging out...", 3000);
    ...
    const handleDeleteSalesMan = async (saleman) => {
        ...
        try {
            const response = await fetch(`http://localhost:5000/auth/delete-user/${saleman._id}`, {
                method: "DELETE",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${localStorage.getItem("token")}`
    ...
    const filteredUsers = salesManUsers.filter((user) =>
    ...
    const exportToPDF = () => {
        ...
        const tableRows = filteredUsers.map(med => [med.name, med.contact, med.email, new Date(med.joinedAt).toLocaleString()]);
    ...
    const handleUpdate = async (e) => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5000/auth/update-user/${SelectedSalesMan._id}`, {
                method: "PUT",
                headers: {
                    Authorization: `Bearer ${localStorage.getItem("token")}`,
                    "Content-Type": "application/json",
            ...
            setSalesManUsers((prev) =>
                prev.map((user) => (user._id === SelectedSalesMan._id ? { ...user, ...updatedSalesMan } : user))
            );
        ...
        <>
            <div className="container">
                ...
                <div className="d-flex justify-content-end " style={{ marginTop: 70 }}>
                    <button className="btn btn-outline-dark position-relative" type="button" onClick={handleCreateAccount}>
                        Create SalesMan Account <i className="fa-solid fa-user-plus mx-2"></i>
                    </button>
                </div>

                <label className="form-label my-3"><b>Search Salesman</b></label>
                <input
                    type="text"
                ...
                <table className="table table-striped table-hover text-center my-4">
                    ...
                    <tbody>
                        ...
                        {filteredUsers.length > 0 ? (
                            filteredUsers.map((user, index) => (
                                <tr key={user._id}>
                                    ...
                                    {isPromoting === "SalesManToAdmin" ? (
                                        <td>
                                            <button
                                                className={`btn btn-${user.role === "admin" ? "danger" : "success"} btn-sm mx-1`}
                                                onClick={() => makeAdmin(user)}
                                            >
                                                <i className={`fas ${user.role === "admin" ? "fa-user-minus" : "fa-user-shield"}`}></i>
                                                {user.role === "admin" ? " Cancel Adminship" : " Make Admin"}
                                            </button>
                                        </td>

                                    ) : (
                                        <td>
                                            <button className="btn btn-outline-success btn-sm mx-1" onClick={() => openEditModal(user)}>
                                                <i className="fas fa-edit"></i>
                                            </button>
                                            <button className="btn btn-outline-danger btn-sm mx-1" onClick={() => handleDeleteSalesMan(user)}>
                            ...
                            <tr>
                                <td colSpan="7" className="text-center">No Sales Man found</td>
                            </tr>
                        )}
                    </tbody>
                </table>
                ...
                <div className="modal-dialog">
                    <div className="modal-content">
                        <div className="modal-header">
                            <h5 className="modal-title" id="editUserModalLabel">Edit Sales Man</h5>
                                ...
                                <form onSubmit={handleUpdate}>
                                    <div className="mb-3">
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => setSelectedSalesMan({ ...SelectedSalesMan, name: e.target.value })}
                                            required
                                        />
                                    </div>
                                    <div className="mb-3">
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => setSelectedSalesMan({ ...SelectedSalesMan, contact: e.target.value })}
                                            required
                                        />
                                    </div>
                                    <div className="mb-3">
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => setSelectedSalesMan({ ...SelectedSalesMan, email: e.target.value })}
                                            required
                                        />
                                    </div>

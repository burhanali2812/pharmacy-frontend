...
function Setting({ user, setUser }) {
  ...
  const handleProfileUpdate = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5000/auth/update-user/${user._id}`, {
        method: "PUT",
        headers: {
  ...
  const openEditModalPassword = () => {
    ...
    const editModal = new window.bootstrap.Modal(document.getElementById("editUserModalPassword"));
    editModal.show();
  };

  const openEditModal = () => {
    setSelectedUser(user);
    const editModal = new window.bootstrap.Modal(document.getElementById("editUserModal"));
    editModal.show();
  };
  ...
  const handleChangePassword = async (e) => {
    ...
    if (newPassword !== newConfirmPassword) {
      showToast("error", "New password and confirm password do not match", 3000);
      return;
    }
    const result = await Swal.fire({
      title: "Are you sure?",
    ...
    try {
      const response = await fetch(`http://localhost:5000/auth/change-password/${user._id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  return (
    <>
      <div className="container mt-4">
        <h2 className="mb-4"><i className="fas fa-cogs"></i> Admin Settings</h2>

        {/* Profile Settings */}
        <div className="card mb-4 shadow-sm">
          <div className="card-body">
            <h5 className="card-title fw-bold"><i className="fas fa-user-edit"></i> Edit Profile</h5>
            ...
            <div className="mb-3">
              <label className="form-label"><i className="fas fa-phone"></i> Contact</label>
              <input type="text" className="form-control" value={contact} onChange={(e) => setContact(e.target.value)} />
            </div>

            {/* Update Profile Button */}
            <div className="text-end">
              <button className="btn btn-primary px-4" onClick={handleProfileUpdate}>
        ...
        <div className="row g-3">
          ...
          <div className="col-md-6">
            <div className="card shadow-sm h-100">
              <div className="card-body d-flex flex-column">
                <h5 className="card-title fw-bold"><i className="fas fa-users-cog"></i> Role Management</h5>

                <div className="d-flex flex-column gap-2 mt-auto">
                  <button className="btn btn-outline-success" onClick={openEditModal}>
                    <i className="fas fa-user-plus"></i> Make New Admin
                  </button>

                  <button className="btn btn-outline-dark" onClick={makeAdminFromSalesMan}>
                    <i className="fas fa-user-shield"></i> Promote Salesman to Admin
                  </button>
                </div>
              </div>
            </div>
          ...
          <div className="col-md-6">
            <div className="card shadow-sm h-100">
              <div className="card-body d-flex flex-column">
                <h5 className="card-title fw-bold"><i className="fas fa-lock"></i> Security Settings</h5>

                <div className="d-flex flex-column gap-2 mt-auto">
                  <button className="btn btn-outline-warning" onClick={openEditModalPassword}>
                    <i className="fas fa-key"></i> Change Password
                  </button>

                  <button className="btn btn-outline-secondary" onClick={handleLogoutAllDevices}>
        ...
        <div className="modal-dialog">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title" id="editUserModalLabel">Make New Admin</h5>
                ...
                <form onSubmit={handleUpdate}>
                  <div className="mb-3 input-group">
                    ...
                    <input type="text" className="form-control" placeholder="Admin Name"
                      value={nname} onChange={(e) => setnName(e.target.value)} minLength={3} required />
                  </div>

                  <div className="mb-3 input-group">
                    <span className="input-group-text"><i className="fas fa-phone"></i></span>
                    <input type="number" className="form-control" placeholder="Contact Number"
                      value={ncontact} onChange={(e) => setnContact(e.target.value)} required />
                  </div>

                  <div className="mb-3 input-group">
                    <span className="input-group-text"><i className="fas fa-envelope"></i></span>
                    <input type="email" className="form-control" placeholder="Email Address"
                      value={nemail} onChange={(e) => setnEmail(e.target.value)} required />
                  </div>
                  ...
                  <div className="modal-footer">
                    ...
                    <button type="submit" className="btn btn-outline-primary">Make Admin</button>
                  </div>
                </form>
              )}
            </div>
      ...
      <div className="modal fade" id="editUserModalPassword" tabIndex="-1" aria-labelledby="editUserModalLabelPassword" aria-hidden="true">
  <div className="modal-dialog">
      ...
      <div className="modal-body">
        {selectedUser && (
          <>
            ...
            <p className="text-muted mb-3">Changing password for: <strong>{selectedUser.name}</strong></p>
              ...
              <div className="mb-3">
                ...
                <div className="input-group">
                  ...
                  <input type="password" className="form-control" placeholder="Enter Old Password"
                    value={oldPassword} onChange={(e) => setOldPassword(e.target.value)} minLength={8} required />
                </div>
              </div>
              ...
              <div className="mb-3">
                ...
                <div className="input-group">
                  ...
                  <input type="password" className="form-control" placeholder="Confirm New Password"
                    value={newConfirmPassword} onChange={(e) => setNewConfirmPassword(e.target.value)} minLength={8} required />
                </div>
              </div>

...
function Dashboard({ medicines, expireMedicines, outOfStock, lowStock, salesManUsers, preExpireMedicines }) {
...
const getTodaySales = async () => {
    try {
        const response = await fetch('http://localhost:5000/auth/sales/today', {
            method: "GET",
            headers: {
...
useEffect(() => {
    ...
    const getSales = async () => {
        try {
            const response = await fetch('http://localhost:5000/auth/getPriceDetails', {
                method: "GET",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${localStorage.getItem("token")}`
    ...
    const cardsData = [
        { id: 1, icon: "fa-coins", title: "Total Sales", count: `Rs ${sales.totalSalesPrice}`, description: "Total sales revenue." },
        { id: 2, icon: "fa-chart-bar", title: "Discounted Profit",  count: `Rs ${Number(sales.profit || 0).toFixed(2)}`, description: "Profit including discounts." },
        { id: 3, icon: "fa-chart-line", title: "Non-Discounted Profit", count: `Rs ${sales.totalSalesPrice - sales.totalActualPrice}`, description: "Profit without discounts." },
        { id: 4, icon: "fa-pills", title: "Total Medicines", count: medicines.length, description: "All available medicines in stock." },
        { id: 5, icon: "fa-exclamation-triangle", title: "Expired Medicines", count: expireMedicines.length, description: "Medicines past their expiry date." },
        { id: 6, icon: "fa-box-open", title: "Low Stock Medicines", count: lowStock.length, description: "Medicines running low on stock." },
        { id: 7, icon: "fa-ban", title: "Out of Stock Medicines", count: outOfStock.length, description: "Completely out-of-stock medicines." },
        { id: 8, icon: "fa-users", title: "Salesmen", count: salesManUsers.length, description: "Total number of sales staff." },
        { id: 9, icon: "fa-truck", title: "Suppliers", count: suppliers.length, description: "Pharmacy suppliers and distributors." },
    ];
                ...
                <div className="row gx-2">
                        ...
                        <div className="card shadow-lg border-0 p-3">
                            <div className="d-flex justify-content-between align-items-center mb-4 mt-5">
                                <h4 className="fw-bold text-dark">
                                    <i className="fa-solid fa-chart-line me-2"></i> Pharmacy Dashboard
                                </h4>
                                <button className="btn btn-outline-dark" onClick={handleSetting}>
                    ...
                    <div className="col-lg-3">
                        ...
                        <div className="card shadow-lg border-0 p-4 bg-dark text-white">
                            ...
                            <h5 className="text-center fw-bold mb-4">
                                <i className="fa-solid fa-exclamation-triangle me-2 text-warning"></i> Expiring Soon
                            </h5>
                            {preExpireMedicines.length > 0 ? (
                                <ul className="list-group list-group-flush">
                                    {preExpireMedicines.slice(0, 5).map((med) => (
                                        <li key={med.id} className="list-group-item bg-dark text-white d-flex justify-content-between">
                                            <span>{med.name}</span>

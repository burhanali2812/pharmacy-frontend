...
function SupplierShow({ role }) {
    ...
    const filteredSupplier = suppliers.filter((sup) =>
        ...
        (sup.licenseNumber && sup.licenseNumber.toLowerCase().includes(searchQuery.toLowerCase()))
    );
    ...
    const handleAddSupplier = async (event) => {
        ...
        try {
            const response = await fetch('http://localhost:5000/auth/add-supplier', {
                method: 'POST',
                headers: {
                    Authorization: `Bearer ${localStorage.getItem("token")}`,
                    "Content-Type": "application/json",
            ...
            if (response.ok) {
                ...
                getSuppliers();
                setAddress(""); setContact(""); setLicenseNumber(""); setName("");
            }
            else {
    ...
    const handleDeleteSupplier = async (supplier) => {
        ...
        try {
            const response = await fetch(`http://localhost:5000/auth/delete-supplier/${supplier._id}`, {
                method: "PUT",
                headers: {
                    Authorization: `Bearer ${localStorage.getItem("token")}`,
                    "Content-Type": "application/json",
            ...
            showToast("success", `Supplier ${supplier.name} deleted successfully`, 3000);
            getSuppliers();
    ...
    const exportToPDF = () => {
        ...
        const tableColumn = ["License Number", "Supplier Name", "Contact", "Medicine Quantity", "Total Cost", "Paid Amount", "Pending Amount"];
        const tableRows = filteredSupplier.map(med => [med.licenseNumber, med.name, med.contact, med.medicineQuantity, med.totalCost, med.paidAmount, med.totalCost - med.paidAmount]);
    ...
    const openEditModal = (supplier) => {
        ...
        const editModal = new window.bootstrap.Modal(document.getElementById("editSupplierModal"));
        editModal.show();
    };
    const handleUpdate = async (e) => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5000/auth/update-supplier/${selectedSupplier._id}`, {
                method: "PUT",
                headers: {
                    Authorization: `Bearer ${localStorage.getItem("token")}`,
                    "Content-Type": "application/json",
            ...
            setSuppliers((prev) =>
                prev.map((sup) => (sup._id === selectedSupplier._id ? updatedSupplier : sup))
            );
    ...
    return (
        <div>
            <div className='container'>
                ...
                <h2 style={{ textAlign: 'center' }} className='my-3 mb-3'>SUPPLIERS</h2>
                <div style={{ marginTop: 70 }}>
                    <div className='container my-3'>
                        <form onSubmit={handleAddSupplier}>
                            <div className="row">
                                <div className="mb-3 col-md-4 mx-auto">
                                    <label htmlFor="licenseNumber" className="form-label"><b>License Number</b></label>
                                    <input type="text" className="form-control" name='licenseNumber' id="licenseNumber" placeholder='Enter Unique License Number' required
                                        value={licenseNumber} onChange={(e) => setLicenseNumber(e.target.value)} />
                                </div>

                                <div className="mb-3 col-md-4 mx-auto">
                                    <label htmlFor="name" className="form-label"><b>Supplier Name</b></label>
                                    <input type="text" className="form-control" name='name' id="name" placeholder='Enter Supplier/Company Name' required
                                ...
                                <div className="mb-3 col-md-8 mx-auto">
                                    ...
                                    <div className="d-flex">
                                        <input type="text" className="form-control" name='address' id="address" placeholder='Enter Physical Address' required
                                ...
                                <div className="mb-3 col-md-4 mx-auto" style={{ marginTop: 30 }}>
                                    <button className="btn btn-outline-success w-100" type="submit" >
                                        Add Supplier <i className="fa-solid fa-boxes-packing mx-2"></i>
                                    </button>
                                </div>
                ...
                <div>
                    <div className="d-flex justify-content-end" style={{ marginTop: 30 }}>
                        <button className="btn btn-outline-dark position-relative" type="button" onClick={handlePastSuppliers}>
                            View Past Suppliers <i className="fa-solid fa-clock-rotate-left mx-2"></i>
                    ...
                    <label className="form-label my-3"><b>Search Suppliers</b></label>
                    <input
                        type="text"
                        ...
                        {filteredSupplier.length > 0 ? (
                            filteredSupplier.map((sup, index) => (
                                <tr key={sup._id}>
                                    ...
                                    <td style={{ color: 'red' }}>Rs: {sup.totalCost - sup.paidAmount}</td>

                                    <td>
                                        <button className="btn btn-outline-success btn-sm mx-1" onClick={() => openEditModal(sup)}>
                                            <i className="fas fa-edit"></i>
                                        </button>
                                        <button className="btn btn-outline-danger btn-sm mx-1" onClick={() => handleDeleteSupplier(sup)}>
                                            <i className="fas fa-trash"></i>
                                        </button>
                                        <button className="btn btn-outline-primary btn-sm mx-1" onClick={() => handleRowClicked(sup._id)}>
                ...
                <div className="modal-dialog">
                    <div className="modal-content">
                        <div className="modal-header">
                            <h5 className="modal-title" id="editSupplierModalLabel">Edit Supplier</h5>
                                ...
                                <form onSubmit={handleUpdate}>
                                    <div className="mb-3">
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => setSelectedSupplier({ ...selectedSupplier, licenseNumber: e.target.value })}
                                            required
                                        />
                                    </div>
                                    <div className="mb-3">
                                    ...
                                    <div className="mb-3">
                                        <label className="form-label">
                                            Enter amount to be paid <b>(Pending Amount: Rs {selectedSupplier.totalCost - selectedSupplier.paidAmount})</b>
                                        </label>
                                        <input
                                            type="number"
                                            className="form-control"

...
function PastSupplier() {
    ...
    useEffect(() => {
     ...
     const handleDeletePermanently = async (passtSupplier) => {
            ...
            try {
                const response = await fetch(`http://localhost:5000/auth/delete-permanent-supplier/${passtSupplier._id}`, {
                    method: "DELETE",
                    headers: {
                        Authorization: `Bearer ${localStorage.getItem("token")}`,
                        "Content-Type": "application/json",
                ...
                showToast("success", `Supplier ${passtSupplier.name} deleted Permanently successfully`, 3000);
                setSuppliers((prev) => prev.filter((sup) => sup._id !== passtSupplier._id));
            } catch (error) {
    ...
    const handleRecoverSupplier = async (supplier) => {
            ...
            if (!result.isConfirmed) return;
            try {
                const response = await fetch(`http://localhost:5000/auth/recover-supplier/${supplier._id}`, {
                    method: "PUT",
                    headers: {
                        Authorization: `Bearer ${localStorage.getItem("token")}`,
                        "Content-Type": "application/json",
                ...
                showToast("success", `Supplier ${supplier.name} recover successfully`, 3000);
    ...
    return (
        <>
            ...
            <h2 style={{ textAlign: 'center' }} className='my-3 mb-3'>PAST SUPPLIERS</h2>
                    ...
                    {filteredSupplier.length > 0 ? (
                        filteredSupplier.map((sup, index) => (
                            <tr key={sup._id}>
                                ...
                                <td style={{ color: 'red' }}>{sup.totalCost - sup.paidAmount}</td>

                                <td>
                                    <button className="btn btn-outline-dark btn-sm mx-1" onClick={()=> handleRecoverSupplier(sup)}>
                                        <i className="fas fa-recycle"></i>
                                    </button>
                                    <button className="btn btn-outline-danger btn-sm mx-1" onClick={()=> handleDeletePermanently(sup)}>

...
function ExpiredMedicines({ expireMedicines }) {
    const [searchQuery, setsearchQuery] = useState("");
    const filteredMedicines = expireMedicines.filter((med) =>
        med.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
        (med.sID && String(med.sID).toLowerCase().includes(searchQuery.toLowerCase()))
    );
    const exportToPDF = () => {
        const doc = new jsPDF();
        doc.text("Expired Medicines Report", 14, 10);

        const tableColumn = ["Medicine ID", "Medicine Name", "Quantity", "Price", "Supplier Name", "Expiry Date"];
        const tableRows = expireMedicines.map(med => [med.sID, med.name, med.quantity, med.price, med.supplier, med.expire]);
        ...
        doc.save("expiredMedicines.pdf");
    };
    const exportToCSV = () => {
        const tableColumn = ["Medicine ID", "Medicine Name", "Quantity", "Price", "Supplier Name", "Expiry Date"];
        const tableRows = expireMedicines.map(med => [
        ...
        saveAs(blob, "expiredMedicines.csv");
    };
    const exportToExcel = () => {
        const tableData = expireMedicines.map(med => ({
            "Medicine ID": med.sID,
        ...
        XLSX.writeFile(workbook, "expiredMedicines.xlsx");
    };
    return (
        <div className='container'>
            <h1 style={{ textAlign: 'center' }} className='my-3'>PHARMACY MANAGEMENT SYSTEM</h1>
            <h2 style={{ textAlign: 'center' }} className='my-3 mb-3'>EXPIRED-MEDICINE</h2>

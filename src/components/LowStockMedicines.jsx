...
function LowStockMedicines({lowStock}) {
   const [searchQuery, setsearchQuery] = useState("");
      const filteredMedicines = lowStock.filter((med) =>
          med.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
          (med.sID && String(med.sID).toLowerCase().includes(searchQuery.toLowerCase()))
      );
      const exportToPDF = () => {
        const doc = new jsPDF();
        doc.text("Low Stock Medicines Report", 14, 10);

        const tableColumn = ["Medicine ID", "Medicine Name", "Quantity", "Price", "Supplier Name", "Expiry Date"];
        const tableRows = lowStock.map(med => [med.sID, med.name, med.quantity, med.price, med.supplier, med.expire]);
        ...
        doc.save("lowStockMedicines.pdf");
    };
    const exportToCSV = () => {
        const tableColumn = ["Medicine ID", "Medicine Name", "Quantity", "Price", "Supplier Name", "Expiry Date"];
        const tableRows = lowStock.map(med => [
        ...
        saveAs(blob, "lowStockMedicines.csv");
    };
    const exportToExcel = () => {
        const tableData = lowStock.map(med => ({
            "Medicine ID": med.sID,
        ...
        XLSX.writeFile(workbook, "lowStockMedicines.xlsx");
    };
      return (
